...
class DB
{
	...
	function DB($base, $server, $user, $pass)
	{
		...
		$this->link = mysqli_connect($server, $user, $pass,$base) or die('Verbindung zur Datenbank konnte nicht hergestellt werden.');
		//mysql_select_db($base)               or die('Datenbank konnte nicht ausgewählt werden.');
		mysqli_query($this->link,"SET NAMES 'utf8'") or die('Database was not able to set UTF-8 mode.');
	...
	{
		$this->nbQueries++;
		$this->lastResult = mysqli_query($this->link,$query) or $this->debug($debug, $query, $result); //$this->debugAndDie($query);
	...
	{
		$this->nbQueries++;
		$result = mysqli_query($this->link,$query) or $this->debug($debug, $query, $result); //$this->debugAndDie($query);
	...
	function fetchNextRowArray($result = NULL, $result_type=MYSQL_ASSOC)
	{
		if ($result == NULL)
		$result = $this->lastResult;
